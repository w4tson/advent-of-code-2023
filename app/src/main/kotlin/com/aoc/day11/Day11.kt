...
data class Universe(val galaxies: List<Coord>, val width: Int, val height: Int) {
    fun expand(expandBy : Int = 1) : Universe {
        val allX = galaxies.map { it.x }
        val allY = galaxies.map { it.y }
        val blankColumns = (0..width -1).filter{ !allX.contains(it.toLong()) }
        val blankRows = (0..height -1).filter{ !allY.contains(it.toLong()) }

        val expandedColumns = blankColumns.reversed().fold(this) { acc, i ->
            val newG = acc.galaxies.map { (x,y) ->
                if (x>i) Coord(x+expandBy-1,y) else Coord(x,y)
            }
        ...
        val fullyExpandedUniverse = blankRows.reversed().fold(expandedColumns) { acc, i ->
            val newG = acc.galaxies.map { (x,y) ->
                if (y>i) Coord(x,y+expandBy-1) else Coord(x,y)
            }
...
fun part1(input : String, expandBy: Int = 1) : Long {
    val sum = input.toUniverse().expand(expandBy)
